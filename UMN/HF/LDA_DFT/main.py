...
def internuclearPotentialElement(basis_1, basis_2):
...
def internuclearPotentialMatrix(basis_functions):
    ...
    for i in range(len(basis_functions)):
        for j in range(len(basis_functions)):
            norm = basis_functions[i].overall_norm * basis_functions[j].overall_norm
            iNPMatrix[i][j] = (norm * 
                internuclearPotentialElement(basis_functions[i], basis_functions[j]))
...
def test():
	...
	attract1 = internuclearPotentialMatrix(basis_sets)
