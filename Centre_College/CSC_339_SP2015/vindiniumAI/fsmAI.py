...
class AI:
    ...
    def canPassThrough(self,position):
        ...
        if position[0] >= 0 and position[1] >= 0 and position[0] < self.game.board_size and (position[1]) < self.game.board_size and \
           (self.game.board_map[position[0]][position[1]] == " " or self.game.board_map[position[0]][position[1]] == "X" or self.game.board_map[position[0]][position[1]] == "$" or position in self.game.heroes_locs):
            return True
        else:
            return False
...
##        print len(inputList)
    ...
    def decide(self):
        ...
        print orderMines
        print orderTaverns
        if self.game.hero.life > 20:
            nextAction = self.pathTo(self.game.hero.pos, orderMines)
        else:
            nextAction = self.pathTo(self.game.hero.pos, orderTaverns)
        #nextAction = []
        #pMoves = self.getCardinalCoordinates(self.game.hero.pos) + [self.game.hero.pos]
        #selection = random.choice(pMoves)
        #while not self.canPassThrough(selection):
        #    selection = random.choice(pMoves)

        #nextAction.append(selection)
        return nextAction[1:]
    """
