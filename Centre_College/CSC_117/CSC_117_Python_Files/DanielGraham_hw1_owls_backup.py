...
def main():
    ...
    if num_owls < 1: #The following two lines take into account the edge values
        ...
        for i in range(num_owls): #This loop begins the action of the program for as many owls as are racing.  It will run as many times as there are owls entered.
            ...
            distance_owl=random.randrange(2,15)
            ...
            for j in range (distance_owl): #This part actually prints the owl j*5 spaces past the end.
                if j%2==0: #To simulate flapping wings, owl image a and b (open wings) had to be printed every other line with owlflap a and b printed.
                    for line in owla: #This line prints the first half of owla, line by line with spaces(j*5) in front of each line.
                        print_spaces(j*5),
                        print line, #The comma prevents the print function from printing an extra \n between each item in list owl
                    for line in owlb: #This line prints the second half of owlb with the same amount of spaces in front.
                        print_spaces(j*5),
                        print line,
                    time.sleep(.25)
                    ...
                    for line in owlflapa:
                        print_spaces(j*5+10),
                        print line,
                    for line in owlflapb:
                        print_spaces(j*5+10),
                        print line,
                    time.sleep(.25)
                    print_blank_lines(60)
            if j<6: #if the owl does not go further than a distance of 35 yards, the program displays a buldger animation in the same way it shows the owl.
                print "As", name_owl, "approached the finish line he could almost taste victory...But suddenly!"
                time.sleep(3)
                print
                print
                print
                for line in bludgera:
                    print_spaces(j*5+20)
                    print line,
                for line in bludgerb:
                    print_spaces(j*5+20)
                    print line,
                for line in bludgerc:
                    print_spaces(j*5+20)
                    print line,
                for line in bludgerd:
                    print_spaces(j*5+20)
                    print line,
                for line in bludgera:
                    print_spaces(j*5+20)
