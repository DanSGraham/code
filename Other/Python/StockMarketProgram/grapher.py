...
#By Daniel Graham
#One issue is that the format of the Stock tiime and the news time is different. Need to coordinate the times
from graphics import *
from better_button import *
#Some headlines are not from the same day. Need to convert the month day information into dd/mm/yy format.
#THERE MAY BE AN ISSUE WITH THE STOCK GETTER. THE STOCKS LOOK WEIRD....Very steep jumps and plateaus.
#Add a browse feature. Browse by headlines and dates.
...
def convert_to_seconds(time):
    ...
    new_time = int(time_list[0][-2:]) * 3600 + int(time_list[1]) * 60 + int(time_list[2][:3])
    return new_time
