...
def log_sigmoid(input_stimulus, slope_param=1.0):
    ...
    sig_val = np.divide(1.0, (1.0 + (np.exp(
                  np.multiply(-1., np.multiply(slope_param, input_stimulus))))))
    return sig_val
...
def cross_entropy_cost(output_vector, target_vector):
    ...
    return -(np.sum(
(np.multiply(target_vector, np.log(output_vector)) + np.multiply((1 - target_vector), np.log(1 - output_vector)))))
...
def Kullback_Leibler_divergence(output_vector, target_vector):
    ...
    return np.multiply(target_vector, np.log(np.divide(target_vector, output_vector)))
