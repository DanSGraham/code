...
class Brain:
    ...
    def learn(self, trainingFile):
        ...
        if self.maxEpochs != None:
            return self.training_method(self.network, input_vals, output_vals, self.maxEpochs, self.bSize, self.tFactor)
        else:
            return self.training_method(self.network, input_vals, output_vals, self.bSize, self.tFactor)
...
def batch_train(network, input_set, output_set, batch_size, train_factor):
    ...
    for k in range(leftover_data):
        ...
        w_err_tmp, b_err_tmp, MSE = network.correction(output_set[upper_limit + k])
        for n in range(len(weights_error)):
            weights_error[n] += w_err_tmp[n]
            bias_error[n] += b_err_tmp[n]
...
def single_epoch(network, input_matrix, output_matrix, minibatch_size, train_factor, train_to_test_ratio = 0.7):
    ...
    tot_SSE = 0.0
    for j in range(int(len(input_matrix) * train_to_test_ratio), len(input_matrix)):
        network.calculate(input_matrix[index_list[j]])
        weight_err, bais_err, MSE = network.correction(output_matrix[index_list[j]])
        tot_SSE += MSE

    return tot_SSE
    
def stochastic_train(network, input_set, output_set, max_num_epochs, batch_size, train_factor):
   
    SSE_list = []
    for i in range(max_num_epochs):
        SSE_list.append(single_epoch(network, input_set, output_set, batch_size, train_factor))
    return SSE_list
